...
#define MC_SAVES_DIR "/saves"
#endif
#define SAVE_PATH MC_SAVES_DIR "/minicraft.map"
#define SAVE_TMP_PATH MC_SAVES_DIR "/minicraft.map.tmp"
...
// --- SDL2 SHIM GLOBALS & HELPERS ---
static SDL_Window *sdl_win = NULL;
static SDL_Renderer *sdl_ren = NULL;
static SDL_Texture *sdl_tex = NULL;
static uint32_t sdl_pixels[MINIOS_DOOM_W * MINIOS_DOOM_H];
static int sdl_kbd_queue[512];
static int sdl_kbd_head = 0, sdl_kbd_tail = 0;
static int sdl_mouse_x = 0, sdl_mouse_y = 0, sdl_mouse_btn = 0, sdl_mouse_wheel = 0;

static void sdl_poll_events(void) {
    SDL_Event e;
    while (SDL_PollEvent(&e)) {
        ...
        if (e.type == SDL_KEYDOWN || e.type == SDL_KEYUP) {
            ...
            int ps2 = -1, is_ext = 0;
            switch(sc) {
                case SDL_SCANCODE_ESCAPE: ps2 = 0x01; break;
                case SDL_SCANCODE_1: ps2 = 0x02; break; case SDL_SCANCODE_2: ps2 = 0x03; break; case SDL_SCANCODE_3: ps2 = 0x04; break;
                case SDL_SCANCODE_4: ps2 = 0x05; break; case SDL_SCANCODE_5: ps2 = 0x06; break; case SDL_SCANCODE_6: ps2 = 0x07; break;
                case SDL_SCANCODE_7: ps2 = 0x08; break; case SDL_SCANCODE_8: ps2 = 0x09; break; case SDL_SCANCODE_9: ps2 = 0x0A; break;
                case SDL_SCANCODE_0: ps2 = 0x0B; break;
                case SDL_SCANCODE_Q: ps2 = 0x10; break; case SDL_SCANCODE_W: ps2 = 0x11; break; case SDL_SCANCODE_E: ps2 = 0x12; break;
                case SDL_SCANCODE_R: ps2 = 0x13; break; case SDL_SCANCODE_T: ps2 = 0x14; break; case SDL_SCANCODE_U: ps2 = 0x16; break;
                case SDL_SCANCODE_I: ps2 = 0x17; break; case SDL_SCANCODE_O: ps2 = 0x18; break; case SDL_SCANCODE_P: ps2 = 0x19; break;
                case SDL_SCANCODE_A: ps2 = 0x1E; break; case SDL_SCANCODE_S: ps2 = 0x1F; break; case SDL_SCANCODE_D: ps2 = 0x20; break;
                case SDL_SCANCODE_F: ps2 = 0x21; break; case SDL_SCANCODE_G: ps2 = 0x22; break; case SDL_SCANCODE_J: ps2 = 0x24; break;
                case SDL_SCANCODE_K: ps2 = 0x25; break; case SDL_SCANCODE_L: ps2 = 0x26; break;
                case SDL_SCANCODE_C: ps2 = 0x2E; break; case SDL_SCANCODE_V: ps2 = 0x2F; break; case SDL_SCANCODE_B: ps2 = 0x30; break;
                case SDL_SCANCODE_N: ps2 = 0x31; break;
                case SDL_SCANCODE_RETURN: ps2 = 0x1C; break; case SDL_SCANCODE_BACKSPACE: ps2 = 0x0E; break;
                case SDL_SCANCODE_SPACE: ps2 = 0x39; break;
                case SDL_SCANCODE_LSHIFT: ps2 = 0x2A; break; case SDL_SCANCODE_RSHIFT: ps2 = 0x36; break;
                case SDL_SCANCODE_LCTRL: ps2 = 0x1D; break; case SDL_SCANCODE_RCTRL: ps2 = 0x1D; break;
                case SDL_SCANCODE_UP: ps2 = 0x48; is_ext = 1; break;
                case SDL_SCANCODE_DOWN: ps2 = 0x50; is_ext = 1; break;
                case SDL_SCANCODE_LEFT: ps2 = 0x4B; is_ext = 1; break;
                case SDL_SCANCODE_RIGHT: ps2 = 0x4D; is_ext = 1; break;
                case SDL_SCANCODE_F11: ps2 = 0x57; is_ext = 1; break;
                default: break;
            }
            if (ps2 >= 0) {
                if (e.type == SDL_KEYUP) ps2 |= 0x80;
                if (is_ext) { sdl_kbd_queue[sdl_kbd_tail] = 0xE0; sdl_kbd_tail = (sdl_kbd_tail + 1) % 512; }
                sdl_kbd_queue[sdl_kbd_tail] = ps2;
                sdl_kbd_tail = (sdl_kbd_tail + 1) % 512;
            }
        }
        if (e.type == SDL_MOUSEMOTION) { sdl_mouse_x += e.motion.xrel; sdl_mouse_y += e.motion.yrel; }
        if (e.type == SDL_MOUSEBUTTONDOWN || e.type == SDL_MOUSEBUTTONUP) {
            int bit = (e.button.button == SDL_BUTTON_LEFT) ? 1 : (e.button.button == SDL_BUTTON_RIGHT) ? 2 : 4;
            if (e.type == SDL_MOUSEBUTTONDOWN) sdl_mouse_btn |= bit; else sdl_mouse_btn &= ~bit;
        }
        if (e.type == SDL_MOUSEWHEEL) { sdl_mouse_wheel += e.wheel.y; }
    }
}
// --- END SDL2 SHIM ---
...
static long s_kbd(void) {
	sdl_poll_events();
	if (sdl_kbd_head == sdl_kbd_tail) return -1;
	int val = sdl_kbd_queue[sdl_kbd_head];
	sdl_kbd_head = (sdl_kbd_head + 1) % 512;
	return val;
}
static long s_kbd_raw(long on) { (void)on; return 0; }
...
static long s_getc_raw(void) { return -1; }
...
static long s_vga(long on) { (void)on; return 0; }
static long s_pal(const unsigned char *p) { (void)p; return 0; }
...
static long s_present(void) {
	if (!sdl_ready)
		return 0;
	sdl_poll_events();
	for (int i = 0; i < MINIOS_DOOM_W * MINIOS_DOOM_H; i++) {
		unsigned char idx = host_fb[i];
		sdl_pixels[i] = (0xFF << 24) | (mc_pal[idx*3] << 16) | (mc_pal[idx*3+1] << 8) | mc_pal[idx*3+2];
	}
	SDL_UpdateTexture(sdl_tex, NULL, sdl_pixels, MINIOS_DOOM_W * 4);
	SDL_RenderClear(sdl_ren);
	SDL_RenderCopy(sdl_ren, sdl_tex, NULL, NULL);
	SDL_RenderPresent(sdl_ren);
	return 0;
}
static long s_title(const char *t) { if(sdl_win) SDL_SetWindowTitle(sdl_win, t); return 0; }
static long s_mouse(int *m) {
	sdl_poll_events();
	m[0] = sdl_mouse_x; sdl_mouse_x = 0;
	m[1] = sdl_mouse_y; sdl_mouse_y = 0;
	m[2] = sdl_mouse_btn;
	m[3] = sdl_mouse_wheel; sdl_mouse_wheel = 0;
	return 0;
}
static long s_zoom(long on) { (void)on; return 0; }
static void s_yield(void) { SDL_Delay(1); }
static long s_pcspk_init(void) { return 0; }
static long __attribute__((unused)) s_tone(long f) { (void)f; return 0; }
...
static void sdl_init_window(void) {
    if (sdl_ready)
        return;
    if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_EVENTS) != 0) {
        fprintf(stderr, "minicraft: SDL_Init failed: %s\n", SDL_GetError());
        return;
    }
    sdl_win = SDL_CreateWindow("Minicraft", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
        MINIOS_DOOM_W * 3, MINIOS_DOOM_H * 3, 0);
    sdl_ren = SDL_CreateRenderer(sdl_win, -1, SDL_RENDERER_ACCELERATED);
    if (!sdl_ren)
        sdl_ren = SDL_CreateRenderer(sdl_win, -1, 0);
    sdl_tex = SDL_CreateTexture(sdl_ren, SDL_PIXELFORMAT_ARGB8888,
        SDL_TEXTUREACCESS_STREAMING, MINIOS_DOOM_W, MINIOS_DOOM_H);
    if (!sdl_win || !sdl_ren || !sdl_tex) {
        fprintf(stderr, "minicraft: SDL window failed: %s\n", SDL_GetError());
        return;
    }
...
static void tick_mob(Pig *p, int id, float dt, long now) {
    int i = id;
    {
        ...
        float pdx, pdy, pd;
        if (!p->alive) {
            int day = day_light >= 0.35f;
            if (now - p->respawn_ms > 10000 &&
                ((p->kind == MOB_CREEP && !day) || (p->kind == MOB_PIG && day)))
...
static void chunk_path(int cx, int cy, char *out, size_t n) {
    snprintf(out, n, MC_SAVES_DIR "/mc_c_%d_%d.bin", cx, cy);
}
...
int main(int argc, char **argv) {
    ...
    fflush(stdout);
    if (autoframes == 0 && zoomframes == 0 && walkframes == 0 && standframes == 0)
        sdl_init_window();
